...
type ApiHandlers = Record<'bootstrap' | 'health' | 'history' | 'historyById' | 'ingredients' | 'ingredientsByPath' | 'meals' | 'mealsById' | 'pantryById' | 'preferencesById' | 'weekly' | 'weeklyEntryById', Handler>
...
function route(pathname: string): Handler | null {
  ...
  if (/^\/api\/ingredients\/[^/]+(?:\/merge)?$/.test(pathname)) return apiHandlers.ingredientsByPath
...
const [bootstrapModule, healthModule, historyModule, historyByIdModule, ingredientsModule, ingredientsByPathModule, mealsModule, mealsByIdModule, pantryByIdModule, preferencesByIdModule, weeklyModule, weeklyEntryByIdModule] = await Promise.all([
  import('../api/bootstrap.js'),
  import('../api/health.js'),
  import('../api/history.js'),
  import('../api/history/[id].js'),
  import('../api/ingredients.js'),
  import('../api/ingredients/[...path].js'),
  import('../api/meals.js'),
...
const apiHandlers: ApiHandlers = {
  ...
  ingredientsByPath: ingredientsByPathModule.default,
