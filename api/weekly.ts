...
export default async function handler(request: Request) {
  ...
  try {
    if (request.method === 'GET') {
      ...
      console.info(`weekly:query:done ${Date.now() - startedAt}ms`)
      const response = json({ id: plans[0].id, startDate: plans[0].start_date, name: plans[0].name, entries: entries.map((entry) => ({ id: entry.id, plannedDate: entry.planned_date, slot: entry.meal_slot, mealOptionId: entry.meal_option_id, locked: entry.locked })) })
      console.info(`weekly:complete ${Date.now() - startedAt}ms`)
      return response
    }
    ...
    const response = json({ ...body, id: plan.id })
    console.info(`weekly:complete ${Date.now() - startedAt}ms`)
    return response
  } catch (error) {
