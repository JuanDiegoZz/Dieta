...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  try {
    ...
    const [created] = await insertRows('ingredients', [{ id: randomUUID(), canonical_name: body.canonicalName.trim(), normalized_name: normalized, category: typeof body.category === 'string' ? body.category : 'other', active: true }])
